    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="section-padding">
        <div className="container-wide">
          <AnimatedSection>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-display font-bold text-gradient mb-8 sm:mb-10">Our values.</h2>
          </AnimatedSection>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {values.map((v, i) => (
              <AnimatedSection key={v.title} delay={i * 0.08}>
                <div className="glass-card-hover rounded-xl p-5 sm:p-6 h-full">
                  <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center mb-4">
                    <v.icon size={20} className="text-primary" />
                  </div>
                  <h3 className="text-base sm:text-lg font-display font-semibold text-foreground mb-2">{v.title}</h3>
      ...
      <section className="section-padding">
        <div className="container-wide">
          <AnimatedSection>
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-display font-bold text-gradient mb-8 sm:mb-10">Leadership.</h2>
          </AnimatedSection>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            {leadership.map((person, i) => (
              <AnimatedSection key={person.name} delay={i * 0.08}>
                <div className="glass-card rounded-xl p-5 sm:p-6 h-full">
                  <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-secondary flex items-center justify-center mb-4">
                    <span className="font-display font-bold text-foreground text-sm sm:text-base">{person.name.split(" ").map(n => n[0]).join("")}</span>
                  </div>
        ...
        <div className="container-tight">
          <AnimatedSection>
            <div className="glass-card rounded-2xl p-8 sm:p-12 md:p-16 text-center">
              <h2 className="text-2xl sm:text-3xl md:text-4xl font-display font-bold text-gradient mb-4">Join our trajectory.</h2>
              <p className="text-sm sm:text-base text-muted-foreground max-w-md mx-auto mb-6 sm:mb-8">We're always looking for exceptional engineers and strategists.</p>
              <Link to="/careers" className="group inline-flex items-center gap-2 px-6 sm:px-7 py-3 sm:py-3.5 rounded-lg bg-primary text-primary-foreground font-medium text-sm hover:bg-primary/90 transition-all">
